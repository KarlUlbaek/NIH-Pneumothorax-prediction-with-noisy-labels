...
if __name__ == '__main__':
    ...
    sys.path.append("cd ..")
    default_im_dir = r"C:\Users\Karlu\Desktop\11\Learning From Noisy Data\archive"
    default_dataset = r"NIH"

    parser = ArgumentParser()
    parser.add_argument('-s','--dataset',default=default_dataset,help='Dataset', choices =['NIH','chexpert','both'])
    parser.add_argument('-p', '--img_dir', default = default_im_dir, help='the folder path of the images',type=str)
    ...
    if args.dataset == 'both' or args.dataset == 'chexpert':
        ...
        for idx, p in enumerate(tqdm(df['Path'])):
            ...
            if not os.path.exists(out_path):
                image = imread(os.path.join(img_data_dir, split[1], split[2], split[3], split[4]))
                image = resize(image, output_shape=(224, 224), preserve_range=True)
                imsave(out_path, image.astype(np.uint8))
