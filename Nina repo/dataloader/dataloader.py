...
class NIHDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return {'image': image, 'label': label, 'id': item}
...
class NIHDataResampleModule(pl.LightningDataModule):
    def __init__(self, img_data_dir,csv_file_img, image_size, pseudo_rgb, batch_size, num_workers,augmentation,
        ...
        self.train_set = NIHDataset(self.img_data_dir,self.df_train, self.image_size, augmentation=False,
    ...
    def train_dataloader(self, subsampler = None):
    ...
    def test_dataloader(self, shuffle=False):
        return DataLoader(self.test_set, self.batch_size, shuffle=shuffle, num_workers=self.num_workers)


    def dataset_sampling(self):

        if (os.path.exists(os.path.join(self.outdir, 'train.version_{}.csv'.format(self.version_no))),
            os.path.exists(os.path.join(self.outdir, 'val.version_{}.csv'.format(self.version_no))),
            os.path.exists(os.path.join(self.outdir, 'test.version_{}.csv'.format(self.version_no)))):

            print("file already exists and is loaded{}".format(self.version_no))

            train_set = pd.read_csv(os.path.join(self.outdir, 'train.version_{}.csv'.format(self.version_no)))
            val_set = pd.read_csv(os.path.join(self.outdir, 'val.version_{}.csv'.format(self.version_no)))
            test_set = pd.read_csv(os.path.join(self.outdir, 'test.version_{}.csv'.format(self.version_no)))
...
class CheXpertDataResampleModule(pl.LightningDataModule):
    def __init__(self, img_data_dir,csv_file_img, image_size, pseudo_rgb, batch_size, num_workers,augmentation,
        ...
        self.disease_pervalence_total, self.disease_pervalence_female, \
            self.disease_pervalence_male = self.get_prevalence()
        ...
        self.disease_prevalence_total_pw, self.disease_prevalence_female_pw, \
            self.disease_prevalence_male_pw = self.get_prevalence_patientwise()
